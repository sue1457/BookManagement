...
async function getBook(bookId) {
    ...
    try {
        ...
        return res.data;  // 책 데이터가 객체로 넘어옴
    } catch (error) {
...
async function main() {
    ...
    // 브라우저에서 id 가져오기
    const bookId = new URL(location.href).searchParams.get('id'); // view를 통해 들어갔을때 url의 가장 뒤에 있는 id 4자리를 가져오는 기능

    // 토큰 체크
    const token = getToken();
    if (token === null) {
        loacation.assign('/login');
        return;
    }

    // 토큰으로 서버에서 나의 정보 받아오기(해당 토큰이 유효한지 확인해보는 작업)
